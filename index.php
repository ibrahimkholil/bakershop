...
$page_column_class = bakershop_page_layout_columns_class($theme_options['ts_blog_layout'], $theme_options['ts_blog_left_sidebar'], $theme_options['ts_blog_right_sidebar']);

if( is_search() ){
	bakershop_breadcrumbs_title(true, true, esc_html__( 'Search Results for: ', 'bakershop' ) . get_search_query());
}
?>
			...
			<?php	
				if( have_posts() ):
					...
					echo '<div class="alert alert-error">';
						echo '<h3>'.esc_html__('Sorry. There are no posts to display!', 'bakershop').'</h3>';
						echo '<p class="ts-aligncenter">'.esc_html__('Try researching for something else.', 'bakershop').'</p>';
					echo '</div>';
